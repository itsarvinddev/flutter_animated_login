...
void main() {
  testWidgets('renders the login screen', (tester) async {
    await tester.pumpWidget(_app(FlutterAnimatedLogin(
  ...
  testWidgets('email typing does not throw and enables the button',
      (tester) async {
    await tester
        .pumpWidget(_app(FlutterAnimatedLogin(onLogin: (_) async => null)));
    await tester.pumpAndSettle();
      ...
      home: Column(children: [
        Expanded(
            child: FlutterAnimatedLogin(
                controller: a, onSignup: (_) async => null)),
        Expanded(
            child: FlutterAnimatedLogin(
                controller: b, onSignup: (_) async => null)),
      ]),
    ));
    await tester.pumpAndSettle();
  ...
  testWidgets('custom messages survive to the signup screen', (tester) async {
    await tester.pumpWidget(_app(FlutterAnimatedLogin(
      ...
      loginConfig: const LoginConfig(
        messages:
            FormMessages(signUp: 'ZZTOP-SIGNUP', signUpShort: 'ZZTOP-LINK'),
      ),
    )));
    await tester.pumpAndSettle();
  ...
  testWidgets('three gradient colours do not throw', (tester) async {
    await tester.pumpWidget(_app(const SizedBox(
      ...
      height: 900,
      child: FlutterAnimatedLogin(
        config: PageConfig(colors: [Colors.red, Colors.orange, Colors.yellow]),
      ),
    )));
    await tester.pumpAndSettle();
    expect(tester.takeException(), isNull);
  ...
  testWidgets('prefill with a national number enables submit and yields E.164',
      (tester) async {
